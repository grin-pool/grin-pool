...
def main():
	...
	(pb_hash, pb_height, pb_nonce, pb_actual_difficulty, pb_net_difficulty, pb_timestamp, pb_found_by, pb_state) = pb
	...
	for ps in pool_shares:
	    ...
	    (ps_height, ps_nonce, ps_difficulty, ps_timestamp, ps_found_by, ps_validated, ps_is_valid, ps_invalid_reason) = ps
	    gs = db.get_grin_share_by_nonce(ps_nonce)
	    if gs == None:
		# XXX NOTE: no payout for shares not accepted by grin node
		continue;
	    (gs_hash, gs_height, gs_nonce, gs_actual_difficulty, gs_net_difficulty, gs_timestamp, gs_found_by, gs_state) = gs
