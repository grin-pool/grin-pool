...
export class MiningGraphComponent extends Component {
  ...
  render () {
    const { miningData, poolBlocksMined, decimals } = this.props
    ...
    return (
      <div className='mainPicture miningGraphContainer'>
        <Nav tabs>
            ...
            <TabContent activeTab={this.state.activeTab} key={graphIterator}>
              <TabPane tabId={graphIterator}>
                <ResponsiveContainer width='100%' height={270}>
                  <LineChart isAnimationActive={false} data={miningData}>
                    ...
                    {Object.keys(miningData).map((worker) => {
                      return <Line key={miningData[worker]} dot={false} yAxisId='left' stroke={COLORS[algo]} dataKey={'gps'} name={`${worker} Graph Rate`} />
                    })}
