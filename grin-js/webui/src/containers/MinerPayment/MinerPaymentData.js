...
export class MinerPaymentDataComponent extends Component {
  ...
  render () {
    ...
    const minerImmatureBalanceSyntax = (!isNaN(minerImmatureBalance) && minerImmatureBalance > 0) ? `${nanoGrinToGrin(minerImmatureBalance * 0.98).toFixed(9)} GRIN` : 'n/a'
    return (
      <div>
        <h4>Payment Info</h4>
        <Table size='sm'>
          <tbody>
            ...
            <tr>
              <td>Total Payouts</td>
              <td>{nanoGrinToGrin(totalPayoutsAmount)} GRIN</td>
            </tr>
            <tr>
